...
def parse_schedule():
    xls_path = os.path.abspath("dev/ORTEGA_SCHEDULE.xls")
    wb = xlrd.open_workbook(xls_path, formatting_info=True)
    sheet = wb.sheet_by_index(0)
    
    # We will build a 2D grid of strings matching the CSV layout for seamless logic
    grid = []
    for rx in range(sheet.nrows):
        row_vals = []
        for cx in range(sheet.ncols):
            val = sheet.cell_value(rx, cx)
            # xlrd floats (like 7.0) are returned, wait, time is a string 7:00?
...
def process_all():
    xls_path = os.path.abspath("dev/ORTEGA_SCHEDULE.xls")
    grid, instructor, semester_ay, s_row, e_row = parse_schedule()
    project_dir = os.path.dirname(os.path.abspath("dev"))
